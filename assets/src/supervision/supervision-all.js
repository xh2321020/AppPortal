...
let filterVm = new Vue({
    ...
    components: {
        comCalendar: require('../components/calendar.vue')
    },
    methods: {
        ...
        changeArea: function (index, type, filter) {
            ...
            console.log(JSON.stringify(this[filter].show))
            if (type == "more") {
        ...
        changeOrg: function (index, type) {
            ...
            area[index].status = "1";
            if (type == "more") {
                let show = this.area.show;
                this.org = {show: show, more: area};
            } else {
                let more = this.area.more;
    ...
    created: function () {
        let _this = this;
        var urls = {
            'supFieldUrl': supervisionRequest["supFieldUrl"],
