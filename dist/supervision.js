...
/***/ function(module, exports, __webpack_require__) {
	...
	var filterVm = new Vue({
	    ...
	    components: {
	        comCalendar: __webpack_require__(20)
	    },
	    methods: {
	        changeTime: function changeTime(ev) {
	            ...
	            if (!btn.hasClass("btn-success") || mark == "custom") {
	                ...
	                resultVm.fetchTransactions(_webconfig.supervisionRequest.searchUrl);
	            }
	        },
	        changeArea: function changeArea(index, type, filter) {
	            ...
	            console.log((0, _stringify2.default)(this[filter].show));
	            if (type == "more") {
	        ...
	        changeOrg: function changeOrg(index, type) {
	            ...
	            area[index].status = "1";
	            if (type == "more") {
	                var show = this.area.show;
	                this.org = { show: show, more: area };
	            } else {
	                var more = this.area.more;
	                this.org = { show: area, more: more };
	            }
	            // console.log(JSON.stringify(area))
	            this.fetchDepts(area[index].id);
	        },
	        fetchDepts: function fetchDepts(pid) {
	            ...
	            $.ajax({
	                type: "get",
	                dataType: "json",
	                url: _webconfig.supervisionRequest.deptUrl + pid,
	                success: function success(result, state, jqxhr) {
	                    ...
	                    _this.scope = {
	                        show: show.concat(result.slice(0, 7)),
	                        more: result.slice(7)
	                    };
	                },
	    ...
	    created: function created() {
	        var _this = this;
	        var urls = {
	            'supFieldUrl': _webconfig.supervisionRequest["supFieldUrl"],
	            "supSourceUrl": _webconfig.supervisionRequest["supSourceUrl"],
	            'orgUrl': _webconfig.supervisionRequest['orgUrl']
	...
	for (var _i7 = 0; _i7 < 8; _i7++) {
	    ...
	    methods: {
	        ...
	        fetchTransactions: function fetchTransactions(url) {
	            ...
	            // for (var name in ["source", "scope"]) {
	            //     options[name] = options[name + "Code"];
	            //     delete options[name + "Code"];
	            ...
	            $.ajax({
	                ...
	                success: function success(result, state, jqxhr) {
	                    ...
	                    var keyList = [];
	                    for (var _i8 = 0, len = result.length; _i8 < len; _i8++) {
	                        var item = result[_i8];
	                        if (item.latestTrace) {
	                    ...
	                    var limit = Number(that.pageSize) || 10;
...
/***/ function(module, exports) {
	...
	var orgUrl = 'http://172.16.51.137:8000/api/v1.0/cache/find/type/org-company'; // 组织机构-公司接口';
	var supFieldUrl = 'http://172.16.51.137:8000/api/v1.0/cache/find/type/area-code'; // 督办领域接口';
	var deptUrl = 'http://172.16.51.137:8000/api/v1.0/cache/find/PID/'; //{组织机构-公司ID} 某公司下面部门接口';
	    ...
	    methods: {
	        changeTime: function changeTime(ev) {
	            ...
	            if (!btn.hasClass("btn-success") || mark == "custom") {
	                ...
	                console.log((0, _stringify2.default)(this.filterOptions));
	                resultVm.fetchTransactions(_webconfig.supervisionRequest.searchUrl);
	            }
	        },
	        changeArea: function changeArea(index, type, filter) {
	            ...
	            if (index == 0) {
	                ...
	                if (area[index].status == "1") {
	                    ...
	                    for (var _i2 in areaCode) {
	                        if (areaCode[_i2] == area[index].diccode) {
	                            areaCode.splice(_i2, 1);
	                        }
	                    }
	                } else {
	                    area[index].status = "1";
	            ...
	            $.ajax({
	                ...
	                success: function success(result, state, jqxhr) {
	                    ...
	                    var doneList = [],
	                        keyList = [],
	                        otherList = [];
	                    for (var _i5 = 0, len = result.length; _i5 < len; _i5++) {
	                        var item = result[_i5];
	                        if (item.latestTrace) {
	                            item.rate = item.latestTrace.rate;
	                            item.latestDesc = item.latestTrace.description;
